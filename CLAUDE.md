...
**The SQLite tree store also uses the `brz_` prefix** (Rust `spark-sqlite` and the Node `better-sqlite3` store), for consistency with the other backends rather than out of necessity: its tables are already `tree`-named, which is what keeps them clear of the main storage's in the file they share. It does track its schema in a `brz_tree_schema_migrations` table rather than the file's `PRAGMA user_version`, which the main storage owns. The main SQLite storage's own tables and the browser **IndexedDB** stores are NOT prefixed — the main storage owns its file, and each IndexedDB database is per-origin isolated (the tree store there uses a separate `-tree` database).
...
Anchored snippet regions (the code between `// ANCHOR:` and `// ANCHOR_END:`) render verbatim in the docs and clip on narrow viewports past ~100 columns. Keep those lines within ~100 cols. Helper code outside anchors is not rendered and may run long.

Native formatters only get you part of the way. `cargo fmt`, `dart format`, and `swift format` wrap code structure (call chains, argument and collection literals), so run them and let them handle that. But they will NOT split a string literal or reflow a comment, and `gofmt`, `dotnet format`, and eslint do not wrap at all. Since many long snippet lines are log/print message strings, wrap those by hand on a best-effort basis in every language: split a long string with the language's concatenation (`+` or adjacent string literals), break object/argument lists one element per line, move a trailing comment above its statement. Never break a single long token (an LNURL, invoice, or address literal): leave it as-is.
...
- Fixing failures in the **CLI matrix** or **Flutter** (which includes Flutter/Dart CLI static analysis) CI jobs. Keep fixes minimal: only make changes needed to pass the build. Do not add new features, flags, or update descriptions.
...
- **Don't document the callers.** A definition's doc (type, enum, variant, function, field) describes the thing and its contract, never who uses it or how specific callers use it. Naming callers inverts the dependency (the abstraction "knowing" its consumers) and the list goes stale the moment a caller is added, removed, or renamed. ✘ a variant doc'd as *"return the error; the X and Y paths recover from it themselves"*; ✓ *"return the error to the caller."* The "why this caller chose this option" belongs at the call site, where the caller documents its own decision. Test: if adding a new caller would force you to edit this comment, the comment is in the wrong place.
